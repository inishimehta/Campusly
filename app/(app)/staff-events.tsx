...
export default function StaffEvents() {
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter((r) => {
      ...
      const blob =
        `${r.title ?? ""} ${r.location ?? ""} ${r.campus ?? ""} ${r.description ?? ""} ${
          r.requestedByEmail ?? ""
        } ${tags}`.toLowerCase();
  ...
  const updateStatus = async (id: string, status: ReqStatus) => {
    try {
      ...
      const reqData = reqSnap.data() as EventRequest;
      ...
      if (status === "approved") {
        ...
        if (existingSnap.empty) {
          await addDoc(collection(db, "events"), {
            ...
            organizer: reqData.organizer ?? reqData.requestedByEmail ?? "Campusly",
            ...
            priceLabel: reqData.isFree ? "" : (reqData.priceLabel ?? ""),
            attending: 0,
  ...
  const confirmReject = (id: string) => {
    Alert.alert("Reject Request", "Are you sure you want to reject this event request?", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Yes, Continue",
        style: "destructive",
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        {loading ? (
          ...
          <View style={styles.emptyCard}>
            ...
            <Text style={styles.emptyText}>Try changing the filter or search text.</Text>
          </View>
        ) : (
          filtered.map((item) => {
            ...
            return (
              <View key={item.id} style={styles.card}>
                ...
                {/* NEW: Displays the image visually so staff can check it */}
                {!!item.imageUrl && (
                  <Image 
                ...
                {!!(item.organizer || item.requestedByEmail || item.requestedBy) && (
                  <Text style={styles.subMeta}>
                    Organizer/Requester: {item.organizer || item.requestedByEmail || item.requestedBy}
                  </Text>
                )}

                {!item.isFree && !!item.priceLabel && (
                  <Text style={styles.subMeta}>💰 Price: {item.priceLabel}</Text>
                )}
...
const styles = StyleSheet.create({
  ...
  tabsScroll: { flexGrow: 0, flexShrink: 0, marginBottom: 4 },
  ...
  card: {
    ...
    borderColor: "#E5E7EB",
    gap: 8,
    overflow: "hidden", // Keeps image rounded properly
  },
  cardImage: {
    width: "100%",
    height: 140,
  ...
  rejectBtnText: { color: "#FFFFFF", fontWeight: "800", fontSize: 14 },
});